...
let EventsService = class EventsService {
    ...
    async findAll(tenantId, status, page = 1, limit = 10) {
    ...
    async findBySlug(tenantId, unitHandle, slug) {
    ...
    async create(userId, unitId, eventData) {
        ...
        if (!unit.wallet) {
            throw new common_1.BadRequestException('Unit wallet not initialized');
        }
        ...
        if (eventData.budgetAmount > availableBudget) {
            throw new common_1.BadRequestException(`Insufficient budget. Available: ${availableBudget}, Requested: ${eventData.budgetAmount}`);
        }
                ...
                await tx.eventConstraint.create({
                    data: {
                        ...
                        targetColleges: eventData.constraints.colleges?.length
                            ? {
                                create: eventData.constraints.colleges.map((collegeId) => ({
                                    collegeId,
                                })),
                            }
                            : undefined,
                        targetCategories: eventData.constraints.categories?.length
                            ? {
                                create: eventData.constraints.categories.map((categoryId) => ({
                                    categoryId,
                                })),
                            }
                            : undefined,
                        targetCourses: eventData.constraints.courses?.length
                            ? {
                                create: eventData.constraints.courses.map((courseId) => ({
                                    courseId,
                                })),
                            }
                            : undefined,
                        targetSpecializations: eventData.constraints.specializations?.length
                            ? {
                                create: eventData.constraints.specializations.map((specializationId) => ({
                                    specializationId,
                                })),
                            }
                            : undefined,
                        targetBatches: eventData.constraints.batches?.length
                            ? {
                                create: eventData.constraints.batches.map((batchId) => ({
                                    batchId,
                                })),
                            }
                            : undefined,
                        targetDivisions: eventData.constraints.divisions?.length
                            ? {
                                create: eventData.constraints.divisions.map((divisionId) => ({
                                    divisionId,
                                })),
                            }
                            : undefined,
    ...
    async update(id, userId, updateData) {
        ...
        if (event.status !== 'DRAFT' && event.status !== 'NEEDS_REVISION') {
            throw new common_1.BadRequestException('Cannot update event in current status');
        }
        const budgetDiff = updateData.budgetAmount - event.budgetAmount;
        return this.prisma.$transaction(async (tx) => {
            if (budgetDiff > 0 && event.unit.wallet) {
                const availableBudget = event.unit.wallet.allocatedBudget - (event.unit.wallet.spentBudget + event.unit.wallet.lockedBudget);
                if (budgetDiff > availableBudget) {
                    throw new common_1.BadRequestException('Insufficient budget for increased amount');
                }
                await tx.unitWallet.update({
                    ...
                    await tx.eventConstraint.create({
                        data: {
                            ...
                            targetColleges: updateData.constraints.colleges?.length
                                ? {
                                    create: updateData.constraints.colleges.map((collegeId) => ({
                                        collegeId,
                                    })),
                                }
                                : undefined,
                            targetCategories: updateData.constraints.categories?.length
                                ? {
                                    create: updateData.constraints.categories.map((categoryId) => ({
                                        categoryId,
                                    })),
                                }
                                : undefined,
                            targetCourses: updateData.constraints.courses?.length
                                ? {
                                    create: updateData.constraints.courses.map((courseId) => ({
                                        courseId,
                                    })),
                                }
                                : undefined,
                            targetSpecializations: updateData.constraints.specializations?.length
                                ? {
                                    create: updateData.constraints.specializations.map((specializationId) => ({
                                        specializationId,
                                    })),
                                }
                                : undefined,
                            targetBatches: updateData.constraints.batches?.length
                                ? {
                                    create: updateData.constraints.batches.map((batchId) => ({
                                        batchId,
                                    })),
                                }
                                : undefined,
                            targetDivisions: updateData.constraints.divisions?.length
                                ? {
                                    create: updateData.constraints.divisions.map((divisionId) => ({
                                        divisionId,
                                    })),
                                }
                                : undefined,
    ...
    async submitForApproval(eventId, userId) {
        const event = await this.prisma.event.findUnique({
            where: { id: eventId },
            include: { unit: true },
        });
        ...
        if (event.createdByUserId !== userId) {
            throw new common_1.ForbiddenException('You can only submit your own events');
        }
    ...
    async approveEvent(eventId, reviewedByUserId, comments) {
        return this.prisma.$transaction(async (tx) => {
            ...
            if (!event || !event.unit.wallet) {
                throw new common_1.NotFoundException('Event or wallet not found');
            }
            if (event.unit.wallet.lockedBudget < event.budgetAmount) {
                throw new common_1.BadRequestException('Insufficient locked budget allocation');
            }
            await tx.unitWallet.update({
                where: { unitId: event.unitId },
                data: {
    ...
    async rejectEvent(eventId, reviewedByUserId, comments) {
        return this.prisma.$transaction(async (tx) => {
            const event = await tx.event.findUnique({
                where: { id: eventId },
                include: { unit: { include: { wallet: true } } },
    ...
    async getEligibleEventsForStudent(studentId, tenantId) {
        ...
        return events.filter((event) => {
            ...
            if (c.targetCategories.length > 0 &&
            ...
            if (c.targetSpecializations.length > 0 &&
    ...
    async getEventsForOrganizer(unitId) {
    ...
    async getEventsPendingApproval(tenantId) {
        return this.prisma.event.findMany({
            where: {
                unit: { tenantId },
                status: {
